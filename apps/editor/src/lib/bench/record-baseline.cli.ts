 ...
 * version-4 baseline to `g3-baseline.json`. Kept separate from the pure
...
if (!reportPath) throw new Error('Pass --p23b-browser-report <path> from /dev/perf/p23b before writing the v4 baseline');
...
console.log(`[record-baseline] P23B workloads ${baseline.workloads?.length ?? 0} | interaction paths ${Object.keys(baseline.interactions ?? {}).length}`);
console.log(
