/**
 * P23.11 Slice 5 — render-safe validation (Issue #6).
 *
 ...
 * overlaps it. Both are properties of the compiled samples, so they are now
 * decided by the canonical acceptance path — the same samples the renderers
 * consume — instead of being discovered while building a mesh.
 *
 ...
 * one file kept byte-identical, so the editor and museum cannot drift; the
 * test below asserts that invariant rather than trusting it.
 */
...
/**
 * Roomless partition Wall whose single control sits at `anchor`. The bow is
	...
	document.walls = [
		{
			...
			centerline: {
				kind: 'auto-bezier',
				interiorAnchors: [{ id: 'anchor:1', point: [anchor[0], anchor[1]] }]
			}
		} as LayoutWall
	];
	return document;
...
function clearanceOf(document: LayoutDocumentWallFirst, thickness = THICK) {
	...
	const sampled = sampleSegment(
		wallCenterlineSegment(wall, [0, 0], [CHORD, 0], 'forward')
	);
	return wallOffsetClearanceFailure(sampled.samples, thickness);
}

describe('P23.11 slice 5 — Issue #6 rejects before commit', () => {
	...
	it('rejects the edit that folds the offset, naming the Wall', () => {
		...
		const plan = planMoveWallCurveAnchor(baseline, 'wall-p', 'anchor:1', [3, 2]);
		expect(plan.kind).toBe('rejected');
		if (plan.kind !== 'rejected') return;
		// The canonical acceptance path owns the refusal: no document or
		// history commit ever sees a Wall the mesh path cannot render.
	...
	it('rejects the edit that necks the Wall into itself', () => {
		const plan = planMoveWallCurveAnchor(thickWallDocument([3, 0.5]), 'wall-p', 'anchor:1', [3, 3]);
		expect(plan.kind).toBe('rejected');
		if (plan.kind !== 'rejected') return;
		expect(plan.rejection.code).toBe('geometry_invalid');
	...
	it('accepts the same edit once the geometry is corrected', () => {
		const plan = planMoveWallCurveAnchor(thickWallDocument([3, 0.5]), 'wall-p', 'anchor:1', [3, 0.2]);
...
describe('P23.11 slice 5 — core and the mesh builders agree', () => {
...
describe('P23.11 slice 5 — dependency direction stays intact', () => {
