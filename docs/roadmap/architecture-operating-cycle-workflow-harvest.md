...
with an archive copy only for multi-file bundles and non-text evidence. (**Owner ruling 2026-09-22
amends OD-4's archive-copy scope to renderable evidence**, keeping its hybrid mechanism — see the
OD-4 blocks in §7.2 and §15 Q4; the OD-4 wording quoted here is preserved as superseded history.)
...
>
> **Owner ruling 2026-09-22 — OD-4 amended (ratified; raised as skill-audit finding 1 at
> `d3252e3`).** The owner ruling **retains OD-4's hybrid mechanism** (path-preserving compact stub +
> exact anchor, single-SHA reachability, tag insurance) and **narrows its archive-copy scope** from
> "multi-file bundles and non-text evidence" to **renderable evidence only** — PNG/SVG/HTML atlases,
> screenshots, plates, measurements, things whose value is that one can look at them. Prose that
> merely sat inside a bundle directory is stubbed rather than copied: measured evidence showed a
> copied prose body buys nothing the anchor does not already guarantee, forks a real duplicate blob
> whenever its links are rewritten, and adds a second link surface.
>
> The paragraph above is kept verbatim as superseded OD-4 history. Authority for the amended rule is
> this ruling plus `.agents/skills/slice-closeout/SKILL.md` §"Closed work (hybrid rule)", which
> applies it; deliberation stays in Git/PR history and is not restated there.
...
4. **Closed-work mechanism** (its semantics are already fixed by the ratified plan: closed
   ...
   → **Amended by owner ruling 2026-09-22** (ratified; raised as skill-audit finding 1 at
   `d3252e3`): the hybrid mechanism is retained; the archive-copy scope narrows to **renderable
   evidence only**. See §7.2.
